...
def gausshermite(k=21, dims=None, dtype=tf.float64):
    ...
    we = tf.reshape(wi * wj / math.pi, dims)
...
class OSI(Layer):
    ...
    def __init__(self, iterations=17, infer_rate=0.01, umin=-1., umax=1., k=3, momentum=0.7, **kwargs):
    ...
    def call(self, inputs, **kwargs):
        ...
        for _ in tf.range(self.iterations):
            ...
            sigma = tf.clip_by_value(sigma + v[..., 1:2, :], clip_value_min=0.02, clip_value_max=10.)
            rou = tf.clip_by_value(rou + v[..., 2:4, :], clip_value_min=-0.95, clip_value_max=0.95)
